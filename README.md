...
- **Cross-domain transfer** (`transfer_study.py`): colouring-trained models tested,
  ...
  domains vs **1/4** for Graph-Q-SAT.
- **Iterations ≠ wall-clock** (`fig_mrir_time`): fewer iterations (MRIR > 1) but
  ~1–7 s/problem vs MiniSat's ms — attention is costlier per decision.
